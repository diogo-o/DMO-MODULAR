...
public sealed class CreateModel : PageModel
{
    ...
    /// <summary>
    /// The machine group token (B/C) of the draft's machine — the automatic email routing of the
    /// send action (B1/B2/B3 → B; C1/C2/C3 → C; anything else <c>null</c> — no guessing). The
    /// recipients/template are resolved by the backend from the group; the page shows the group
    /// as state information, never a list selection.
    /// </summary>
    public string? PdfMachineGroup { get; private set; }
    ...
    private async Task LoadDraftAsync(Guid pesoId, CancellationToken cancellationToken)
    {
        ...
        // traversal facts). The machine's group (B/C) routes the automatic email send.
        ...
        if (sheet.Production is { } production)
        {
            var group = EmailMachineGroupTokens.FromMachine(
                MachineCode.Parse(production.Machine));
            PdfMachineGroup = group is { } resolved
                ? EmailMachineGroupTokens.ToToken(resolved)
                : null;
        }

        Strip = sheet.Production is { } draftProduction
            ? new ProductionStripModel(
                draftProduction.Reference,
                draftProduction.ProductionNumber,
                draftProduction.Machine,
                ...
                ProductionDate: draftProduction.ProductionDate,
